...
export function BentoReminder() {
  ...
  return (
    <div className="relative w-full h-full overflow-hidden bg-white rounded-2xl flex items-center justify-center p-2 sm:p-0">
      {/* Floating bell notifications - static on mobile */}
      {bellPositions.map((pos, index) => (
        <motion.div
          key={index}
          className={index > 2 ? "absolute hidden sm:block" : "absolute"}
          style={{ left: pos.x, top: pos.y }}
          initial={{ opacity: isMobile ? 0.8 : 0, scale: isMobile ? 1 : 0, y: isMobile ? 0 : 20 }}
          animate={isMobile ? { opacity: 0.8, scale: 1, y: 0 } : {
            opacity: [0, 1, 1, 0],
            scale: [0, 1, 1, 0.8],
            y: [20, 0, 0, -10],
          }}
      ...
      {/* Pulsing ring effect with gradient - hidden on mobile */}
      {!isMobile && (
        <motion.div
